...
#[As_Command(name: 'sanctum:prune-expired')]
class Prune_Expired extends Command
{
    /**
    ...
    public function handle()
    {
        ...
        $this->components->task('Pruning tokens with expired expires_at timestamps', fn() => $model::where('expires_at', '<', now()->sub_hours($hours))->delete());
        if ($expiration = config('sanctum.expiration')) {
            $this->components->task('Pruning tokens with expired expiration value based on configuration file', fn() => $model::where('created_at', '<', now()->sub_minutes($expiration + $hours * 60))->delete());
        } else {
